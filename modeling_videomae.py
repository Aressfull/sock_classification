...
def get_frame_embedding(n_frames, d_hid, num_patches_per_frame):
    """Sinusoid position encoding table"""
    # TODO: make it with torch instead of numpy
    # Frame embedding only use sine? investigate different method
    def get_position_angle_vec(position):
        return [position / np.power(10000, 2 * (hid_j // 2) / d_hid) for hid_j in range(d_hid)]

    sinusoid_table = np.array([get_position_angle_vec(pos_i) for pos_i in range(n_frames+1)])
    sinusoid_table[:, 0::2] = np.sin(sinusoid_table[:, 0::2])  # dim 2i
    sinusoid_table[:, 1::2] = np.cos(sinusoid_table[:, 1::2])  # dim 2i+1

    result = []
    result.append(sinusoid_table[0])
    for j in range(1,n_frames+1):
        for _ in range(num_patches_per_frame):
            result.append(sinusoid_table[j])
    result = np.array(result)

    return torch.FloatTensor(result).unsqueeze(0)

# def get_frame_embedding(n_frames,d_hid, num_patches_per_frame):
#     frame_val = nn.Embedding(n_frames, d_hid).weight
#     result = torch.repeat_interleave(frame_val, num_pathces_per_frame, dim=0)
#     return torch.FloatTensor(result).unsqueeze(0)

def get_sensor_embedding(n_frames, d_hid, num_patches_per_frame):
    """Sinusoid position encoding table"""
    # TODO: make it with torch instead of numpy
    # Frame embedding only use sine? investigate different method
    def get_position_angle_vec(position):
    ...
    sinusoid_table = np.array([get_position_angle_vec(pos_i) for pos_i in range(num_patches_per_frame+1)])
    sinusoid_table[:, 0::2] = np.sin(sinusoid_table[:, 0::2])  # dim 2i
    sinusoid_table[:, 1::2] = np.cos(sinusoid_table[:, 1::2])  # dim 2i+1
    ...
    for _ in range(n_frames):
        for j in range(1,num_patches_per_frame+1): # sensor embedding needs to be different reverse loops
            result.append(sinusoid_table[j])
    result = np.array(result)
...
class VideoMAEEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.cube_frames = (config.num_frames//config.tubelet_size)
        self.num_patches_per_frame = (config.image_size // config.patch_size) ** 2
        # fixed sin-cos embedding
        self.position_embeddings = nn.Parameter(get_sinusoid_encoding_table(self.num_patches + 1, config.hidden_size))
        self.frame_embeddings = nn.Parameter(get_frame_embedding(self.cube_frames,config.hidden_size,self.num_patches_per_frame))
        self.sensor_embeddings = nn.Parameter(get_sensor_embedding(self.cube_frames,config.hidden_size,self.num_patches_per_frame))
        self.config = config
    ...
    def forward(self, pixel_values, bool_masked_pos):
        ...
        embeddings = embeddings + self.frame_embeddings.type_as(embeddings).to(embeddings.device).clone().detach()
        # add sensor embeddings
        embeddings = embeddings + self.sensor_embeddings.type_as(embeddings).to(embeddings.device).clone().detach()
...
class VideoMAEForPreTraining(VideoMAEPreTrainedModel):
    def __init__(self, config):
        ...
        self.position_embeddings = get_sinusoid_encoding_table(
            self.videomae.embeddings.num_patches + 1, config.decoder_hidden_size
        )
        self.frame_embeddings = get_frame_embedding(
            self.cube_frames,config.decoder_hidden_size,self.num_patches_per_frame
        )
        self.sensor_embeddings = get_sensor_embedding(
            self.cube_frames,config.decoder_hidden_size,self.num_patches_per_frame
        )

        self.decoder = VideoMAEDecoder(config, num_patches=self.videomae.embeddings.num_patches)

        self.frame_layer = nn.Linear(config.hidden_size*2, 1, bias=False)
        self.comparisons = config.comparisons
        self.beta = config.beta
        print("Initialised pretraining model with comparisons " + str(self.comparisons) + "and beta " + str(self.beta))
    ...
    def forward(
        ...
        loss_fct = MSELoss()
        sequence_output = outputs[0] #32x可见patchx768

        # Two test add position embeddings and no add
        mask_ratio = (self.num_patches_per_frame*self.cube_frames - sequence_output.shape[1])/(self.num_patches_per_frame*self.cube_frames) #从bool_mask_pos算出mask ratio
        comparisons = self.comparisons
        frame_labels = []
        compare1 = []
        compare2 = []
        for i in range(sequence_output.shape[0]):
            to_compare1 = np.random.choice(sequence_output.shape[1],comparisons,replace=False) #从可见选择patch
            to_compare2 = np.random.choice(sequence_output.shape[1],comparisons,replace=False)
            while ((to_compare2//(self.num_patches_per_frame*(1-mask_ratio)) == to_compare1//(self.num_patches_per_frame*(1-mask_ratio))).all()): #保证选择的patch不在同一个frame
                to_compare2 = np.random.choice(sequence_output.shape[1],comparisons,replace=False)

            frame_labels.append(torch.tensor(to_compare1//(self.num_patches_per_frame*(1-mask_ratio)) <
                                        to_compare2//(self.num_patches_per_frame*(1-mask_ratio))).float().requires_grad_().to(pixel_values.device)) #算label
            compare1.append(sequence_output[i,to_compare1].requires_grad_().to(pixel_values.device))
            compare2.append(sequence_output[i,to_compare2].requires_grad_().to(pixel_values.device))

        frame_labels = torch.stack(frame_labels).requires_grad_().to(pixel_values.device)
        compare1 = torch.stack(compare1).requires_grad_().to(pixel_values.device)
        compare2 = torch.stack(compare2).requires_grad_().to(pixel_values.device)
        combined = torch.cat((compare1,compare2),axis=2).requires_grad_().to(pixel_values.device)
        frame_logit = self.frame_layer(combined).requires_grad_().to(pixel_values.device)
        frame_logit = (frame_logit.sigmoid()).requires_grad_().float().squeeze().to(pixel_values.device)

        sequence_output = self.encoder_to_decoder(
            sequence_output
        )  # [batch_size, num_visible_patches, decoder_hidden_size] 32x可见patch0.9x384
        ...
        expanded_frame_embeddings = self.frame_embeddings.expand(batch_size, -1, -1).type_as(pixel_values)
        expanded_frame_embeddings = expanded_frame_embeddings.to(pixel_values.device).clone().detach()
        frame_emb_visible = expanded_frame_embeddings[~bool_masked_pos].reshape(batch_size, -1, num_channels)
        frame_emb_mask = expanded_frame_embeddings[bool_masked_pos].reshape(batch_size, -1, num_channels)

        expanded_sensor_embeddings = self.sensor_embeddings.expand(batch_size, -1, -1).type_as(pixel_values)
        expanded_sensor_embeddings = expanded_sensor_embeddings.to(pixel_values.device).clone().detach()
        sensor_emb_visible = expanded_sensor_embeddings[~bool_masked_pos].reshape(batch_size, -1, num_channels)
        sensor_emb_mask = expanded_sensor_embeddings[bool_masked_pos].reshape(batch_size, -1, num_channels)

        # [batch_size, num_patches, decoder_hidden_size]
        x_full = torch.cat([sequence_output + pos_emb_visible + frame_emb_visible + sensor_emb_visible,
                            self.mask_token + pos_emb_mask + frame_emb_mask + sensor_emb_mask], dim=1)
...
#             std = torch.as_tensor(IMAGENET_DEFAULT_STD).to(device)[None, None, :, None, None]
            mean = torch.as_tensor([536.4649528748312,536.4649528748312]).to(device)[None, None, :, None, None]
            std = torch.as_tensor([46.81434484100088,46.81434484100088]).to(device)[None, None, :, None, None]
            ...
            labels = videos_patch[bool_masked_pos[:,1:]].reshape(batch_size, -1, num_channels)

        # can add lambda for frame_loss
        loss = loss_fct(logits, labels) + (self.beta*loss_fct(frame_logit,frame_labels))
